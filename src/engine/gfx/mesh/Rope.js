 ...
 *```js
 * for (var i = 0; i < 20; i++) {
 *     points.push(new Vector(i * 50, 0));
 * };
 * var rope = new Rope(Texture.fromImage("snake.png"), points);
 *  ```
 *
 * @class
 * @extends mesh.Mesh
...
class Rope extends Mesh {
  constructor(texture, points) {
    super(texture);
      ...
      /*
       * @member {Vector[]} An array of points that determine the rope
       */
    this.points = points;
  ...
  refresh() {
    ...
    var offset = new Vector(textureUvs.x0, textureUvs.y0);
    var factor = new Vector(textureUvs.x2 - textureUvs.x0, textureUvs.y2 - textureUvs.y0);
